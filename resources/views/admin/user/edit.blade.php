...
@section('page-heading','Edit User Data')
@section('content')
<div class="max-w-7xl mx-auto bg-white p-8 rounded-xl shadow-md border border-gray-100">
    <div class="flex justify-between">
        <a href="{{ route('users.index') }}" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-700 hover:text-white bg-white border border-gray-300 rounded-lg hover:bg-red-500 transition-colors">
  ...
  <form class="space-y-5" action="{{ route('users.update', $user->id) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <!-- Password -->
    <div>
      <label for="password" class="block text-sm font-semibold text-gray-700 mb-1">Password <span class="text-xs text-gray-400">(Leave blank to keep current)</span></label>
    ...
    <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-100">
      <a href="{{ route('users.index') }}" class="px-5 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all">
        Cancel
      </a>
